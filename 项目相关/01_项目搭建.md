...
#####1、在开发中，有时候我们需要根据不同的环境设置不同的环境变量，常见的有三种环境
...
- 方式一：手动修改不同的变量，在 config.ts 文件中
- 方式二：根据 process.env.NODE_ENV 的值进行区分，在 config.ts 文件中
- 方式三：编写不同的环境变量配置文件；分别编译，使用在在 config.ts 文件中

## 四. 区分不同环境

#####1、tsconfig.json 文件配置
#####2、shims-vue.d.ts 文件配置
#####3、defineComponent 作用：限制和类型推导的作用
